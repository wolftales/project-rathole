...
function copy_source
{
  ...
  # M-39104: This block needs to come after the memory adjustments above due to a regex conflict that would happen when
  #  CORES=24 and MEMORY_GB is specified.
  #  Having CORES=24 would result in "<rasd:VirtualQuantity>24</rasd:VirtualQuantity>" being set by the CPU-regex below
  #  which would then get overridden with
  #  '-e "s%<rasd:VirtualQuantity>24</rasd:VirtualQuantity>%<rasd:VirtualQuantity>24576</rasd:VirtualQuantity>%"' above
  #  which would later get overridden with
  #  '-e "s%<rasd:VirtualQuantity>24576</rasd:VirtualQuantity>%<rasd:VirtualQuantity>${memory_mb}</rasd:VirtualQuantity>%".
  #  This would cause the CPU cores to be set to MEMORY_MB. Therefore, the CPU block has been moved to after the memory
  #  regex replacements to prevent the CORES=24 value from being overridden.
  if [[ $CORES != '' ]]; then
