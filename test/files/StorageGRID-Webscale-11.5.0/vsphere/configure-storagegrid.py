...
def wait_for_primary_admin():
    ...
    while waiting:
        try:
            ...
            log.warning('Waiting for primary Admin Node: ' + gmi_api.address + ' (cannot connect)')
            ...
            if e.status == 503:
                log.warning('Waiting for primary Admin Node: ' + gmi_api.address + ' (received 503)')
                if time() >= epoch_timeout:
                    raise AdminTimeoutError('Timed out waiting for primary Admin Node.')
                sleep(args.poll_time)
            elif e.status == 405:
            ...
            log.good('Connection verified (received ' + str(gmi_api.status) + ')')
            waiting = False
    gmi_api.quiet = False
...
def create_sites(config_sites):
    ...
    for new_site in config_sites:
        if new_site['name'] in data.keys():
            log.warning('Site name "' + new_site['name'] + '" Already exists... Not creating.')
        else:
            try:
...
def update_nodes(config_nodes, site_data):
    ...
    while not all_nodes_registered:
        ...
        for addr, node_config in config_nodes.items():
            ...
            if node_data:
                ...
                log.warning('Waiting for "' + addr + '" to register.')
    ...
    if not args.ignore_extra_nodes:
        ...
        if extra_nodes:
            log.warning('Warning: There were unexpected nodes registered with the primary Admin Node:')
            for node in extra_nodes:
                ...
                print_hashed(['An unexpected node has registered with the primary Admin Node.',
                              ...
                              'or execute this script with --ignore-extra-nodes.'], color=Log.yellow)
...
def start_and_monitor_install(start=True, status=None):
    ...
    while not status or status['data']['inProgress']:
        log.warning('StorageGRID provisioning is in progress.')
        sleep(args.poll_time)
        ...
        try:
            ...
            if e.status == 405:
                print_hashed(['StorageGRID has been configured and installed.'], color=Log.green)
                sys.exit(0)
            else:
    ...
    while True:
        ...
        try:
            ...
            log.info(time_string, lf=False)
            log.warning('Retrieving node information')
            nodes = json.loads(gmi_api.call('GET', 'nodes'))
        except GmiApi.ResponseError as e:
            if e.status == 405:
                try:
                ...
                log.warning('The StorageGRID install API is restarting or not responding.')
            ...
            if primary_admin_waiting_for_confirmation(nodes['data']):
                ...
                while True:
                    ...
                    try:
                        ...
                        log.warning('The StorageGRID install web server is restarting or not responding.')
...
def recovery_package_verify():
    tries = 3
    while tries and not recovery_package_verify.status:
        tries -= 1
        try:
            # Call the recovery-package API; downloads a zip file
            gmi_api.call('GET', 'recovery-package', timeout=30)
            # Save the zip file
            ...
            log.good("Downloaded recovery package {}".format(recovery_package_verify.saved_file))
            recovery_package_verify.status = True
        except Exception as e:
            log.quiet_exception('Error occured while saving the recovery package:')
            if tries:
                sleep(args.poll_time)

    if not recovery_package_verify.status:
        print_hashed(['There was an error retrieving the "recovery package" zip file.',
                      'Direct your browser to: https://' + gmi_api.address +
                      '/install/#/install/status',
                      'and follow the instructions.'], color=Log.yellow)
        raise DataInconsistencyError('Recovery package not saved.')
...
def recovery_package_confirm():
    ...
    print_hashed(['The StorageGRID "recovery package" has been downloaded as:',
                 ...
                 'StorageGRID node recovery.'], color=Log.green)
    ...
    if len(nodes_data) >= len(last_nodes_data):
        ...
        for node in nodes_data:
            try:
                ...
                log.quiet_exception('Error processing the "nodes_data":')
            i += 1
    ...
    for node in nodes_data:
        if node['configured'] is False:
            print_hashed(['Node ' + node['name'] + ' was registered with the',
                          ...
                          'perform an expansion procedure.'], color=Log.yellow)
...
class GmiApi:
    ...
    def call(self, method, api, body=None, timeout=5):
        '''Call the GMI API.
        ...
        while retries:
            ...
            if self.verbose or not self.quiet:
                ...
                log.info(time_string + 'Performing ' + method + ' on ' + api + '... ', lf=False)
            ...
            try:
                ...
                if resp.getcode() >= 500 and resp.getcode() < 600:
                    ...
                    if retries:
                        if self.verbose or not self.quiet:
                            # Informational output
                            log.warning('    Retrying on status ' + str(resp.getcode()))
                ...
                if retries:
                    if self.verbose or not self.quiet:
                        # Informational output
                        log.warning('    Retrying on URLError (' + str(e) + ')')
        ...
        if self.data:
            ...
            if self.headers['Content-Type'] == 'application/json':
                ...
                try:
                    ...
                    log.warning("Failed to decode response data: " + str(e))
                # Log the data
...
class Log:
    ...
    def quiet_exception(self, *msgs, **kwargs):
...
def main():
    ...
    # Catch 22, we need "log" defined in case parse fails
    ...
    if status['data']['inProgress'] or status['data']['complete']:
        if args.join_install:
            print_hashed(['Deployment is already in progress.',
                          ...
                          'Monitoring deployment in progress.'], color=Log.yellow)
    ...
    if args.skip_configure:
        log.warning('Option --skip-configure was specified.')
        log.warning('Skipping configuration.')
    else:
        ...
        print_hashed(['Configuration has finished successfully.'], color=Log.green)
    ...
    if args.start_install:
        ...
        print_hashed(['If you are satisfied with this configuration,',
                      'execute the script with --start-install.'], color=Log.green)
...
if __name__ == '__main__':
    ...
    try:
        ...
        log.error('Error: Unable to communicate with the primary Admin Node.')
        log.error('  Received: ' + str(e))
        exit(1)
    except GmiApi.ResponseError as e:
        log.error('Error: Received unexpected return code from the API: ' + str(e.status))
        try:
            ...
            if 'message' in response and 'text' in response['message']:
                log.error('  ' + response['message']['text'])
